...
import { UnbilledAppointmentsFiltersComponent } from './unbilled-appointments-filters/unbilled-appointments-filters.component';
...
@Component({
  selector: 'app-unbilled-appointments',
  templateUrl: './unbilled-appointments.component.html',
  styleUrls: ['./unbilled-appointments.component.css'],
})
export class UnbilledAppointmentsComponent implements OnInit {
  ...
  unbilledAppointmentsListSubject: UnbilledAppointmentsListSubject;
  ...
  constructor(
    ...
    this.unbilledAppointmentsListSubject = new UnbilledAppointmentsListSubject(
      this.reportingService
    );

    this.isSubjectLoading$ = this.unbilledAppointmentsListSubject.getLoading();

    this.view = this.unbilledAppointmentsListSubject.pipe(
      map((data) => {
        let result = {
          data: data,
          total: this.unbilledAppointmentsListSubject.getCount(),
        };
  ...
  loadAppointments() {
    ...
    this.mySelection = [];
    this.unbilledAppointmentsListSubject.getAll(filter);
  }
  ...
  onPageChange(event: PageChangeEvent): void {
    ...
    if (
      ...
      this.unbilledAppointmentsListSubject.getCount() > 1000
    ) {
  ...
  removeRowsFromGrid(idsToRemove: number[]) {
    ...
    this.mySelection = [];
    const currentData = this.unbilledAppointmentsListSubject.getValue();
    const updatedData = currentData.filter(
      (item) => !idsToRemove.includes(item.id)
    );
    this.unbilledAppointmentsListSubject.setCount(updatedData.length);
    this.unbilledAppointmentsListSubject.next(updatedData);
  }
  ...
  exportToExcel() {
    ...
    this.unbilledApptService.exportToExcel(filter).subscribe((res) => {
      if (res) {
        ...
        a.href = url;
        a.download = 'Unbilled Appointments report.xlsx'; // File name for the download
  ...
  getPageStart(total: number): number {
    ...
    if (this.gridState && this.gridState.take === 0) {
      const subjAny = this.unbilledAppointmentsListSubject as any;
  ...
  breadcrumbs: Breadcrumb[] = [
    ...
    },
    {
      ...
      url: '/billing/reporting/unbilled-appointments',
      tabIndex: 2,
      isReportsPage: true,
    },
  ];
