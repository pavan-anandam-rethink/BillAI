...
export class CalendarPopupComponent implements OnInit, OnDestroy{
    /**
     * Accepts either an ElementRef or a native DOM element as anchor
     */
    ...
    private contains(target: any): boolean {
        // Accept both ElementRef and native element for anchor
        let anchorElem: HTMLElement | null = null;
        if (this.anchor instanceof ElementRef) {
            anchorElem = this.anchor.nativeElement;
        } else if (this.anchor instanceof HTMLElement) {
            anchorElem = this.anchor;
        }
        if (!anchorElem) {
            console.warn('CalendarPopupComponent: anchor is not set or invalid. Popup may not position correctly.');
            return false;
        }
        return anchorElem.contains(target) ||
