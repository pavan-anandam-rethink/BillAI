import { Component, Input, NgZone, OnDestroy, ViewChild, OnInit, Output, EventEmitter, OnChanges, SimpleChanges } from '@angular/core';
import {
...
@Component({
    selector: 'patient-claim-details',
    templateUrl: './patient-claim-details.component.html',
    styleUrls: ['./patient-claim-details.component.css']
})

export class PatientClaimDetailsComponent implements OnInit, OnDestroy, OnChanges {
    _paymentId: number;
    ...
    _isDataItemChecked: boolean;
    @Input() set isDataItemChecked(value: boolean) {
        this._isDataItemChecked = value;
    }
    _isLinkedDataItemChecked: boolean;
    @Input() set isLinkedDataItemChecked(value: boolean) {
        this._isLinkedDataItemChecked = value;
    }
    ...
    @Output() allLinkedLinesChangeEmitter = new EventEmitter<any>();
    ...
    selectedLinesChanged(event: any = null): void {
        if (event !== null) {
            ...
            if (calculatedLength == this.gridData.length) {
                ...
                this.allLinkedLinesChangeEmitter.emit(emitModel);
            } else {
                this._selectAllLines = false;
            }
    ...
    loadData(params: PatientClaimDetailsFilterSort): void {
        ...
        this.PatientClaimPostingDetailsSubject.getAllLinked(params);
        this._selectAllLines = false;
    }
