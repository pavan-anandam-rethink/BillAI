...
/****** Object:  StoredProcedure [dbo].[GetClaimsByAccountInfoId]    Modified Date: 1-09-2025 By Jeetendra Negi ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE OR ALTER PROCEDURE [dbo].[GetClaimsByAccountInfoId]
--EXEC [dbo].[GetClaimsByAccountInfoId] 18421,0,10,'',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,6,0,NULL,NULL,''
(
	@AccountInfoId int,     
	@Skip int,
	@Take int, 
...
WITH CTE_ReasonCodeForDeniedClaim AS (
   SELECT DISTINCT pc.hcClaimId,
		  STRING_AGG(pcladj.adjustmentReasonCode, ', ') AS ReasonCodes, pc.id
   ...
   WHERE pc.dateDeleted IS NULL
         ...
         AND pc.claimStatus = 4 -- we need DeniedReason for only Denied Claims
   GROUP BY pc.hcClaimId,pc.id
),
...
SELECT DISTINCT
	  ...
	  CASE 
		...
		ELSE 
			ISNULL(cce3.Charges, 0) - ISNULL(calcNew.TotalPayment, 0) - ISNULL(calc2.WriteOffAmount,0)
...
LEFT JOIN (SELECT hcClaimId FROM [ClaimNotes]) AS cn ON c.Id = cn.hcClaimId 
	...
	(@Tab = 2 AND ([c].[hcClaimStatusId] = 2 OR [c].[hcClaimStatusId] = 7 OR [c].[hcClaimStatusId] = 10 OR [c].[hcClaimStatusId] = 14) AND [c].[isFlagged] = 0) OR
