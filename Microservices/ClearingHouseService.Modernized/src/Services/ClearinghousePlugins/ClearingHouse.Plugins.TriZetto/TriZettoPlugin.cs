...
namespace ClearingHouse.Plugins.TriZetto;

public class TriZettoPlugin : IClearinghousePlugin, IDisposable
{
    private readonly TriZettoConfiguration _config;
    private readonly ILogger<TriZettoPlugin> _logger;
    private readonly AsyncRetryPolicy _retryPolicy;

    public string ClearinghouseId => "TriZetto";
    public string DisplayName => "TriZetto Provider Solutions";
    public bool IsEnabled => _config.IsEnabled;

    public TriZettoPlugin(TriZettoConfiguration config, ILogger<TriZettoPlugin> logger)
    {
        _config = config;
        _logger = logger;
    ...
    public async Task<Result> ValidateConnectionAsync(CancellationToken cancellationToken = default)
    {
        return await _retryPolicy.ExecuteAsync(async () =>
        {
            ...
            var result = client.IsConnected ? Result.Success() : Result.Failure("TriZetto connection failed");
            client.Disconnect();
            return result;
        });
    }
    ...
    public async Task<Result<SubmissionResult>> SubmitClaimAsync(ClaimSubmissionRequest request, CancellationToken cancellationToken = default)
    {
        return await _retryPolicy.ExecuteAsync(async () =>
        {
            ...
            _logger.LogInformation("Submitted {FileName} to TriZetto", request.FileName);
    ...
    {
        return Task.FromResult(Result.Failure<EligibilityResponse>("TriZetto eligibility requires dedicated API integration"));
    }
...
public class TriZettoConfiguration
{
