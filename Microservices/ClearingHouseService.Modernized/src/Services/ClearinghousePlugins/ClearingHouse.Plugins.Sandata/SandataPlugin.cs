...
namespace ClearingHouse.Plugins.Sandata;

public class SandataPlugin : IClearinghousePlugin, IDisposable
{
    private readonly SandataConfiguration _config;
    private readonly ILogger<SandataPlugin> _logger;
    private readonly AsyncRetryPolicy _retryPolicy;

    public string ClearinghouseId => "Sandata";
    public string DisplayName => "Sandata Technologies";
    public bool IsEnabled => _config.IsEnabled;

    public SandataPlugin(SandataConfiguration config, ILogger<SandataPlugin> logger)
    {
        _config = config;
        _logger = logger;
    ...
    public async Task<Result> ValidateConnectionAsync(CancellationToken cancellationToken = default)
    {
        return await _retryPolicy.ExecuteAsync(async () =>
        {
            ...
            var result = client.IsConnected ? Result.Success() : Result.Failure("Sandata connection failed");
            client.Disconnect();
            return result;
        });
    }
    ...
    public async Task<Result<SubmissionResult>> SubmitClaimAsync(ClaimSubmissionRequest request, CancellationToken cancellationToken = default)
    {
        return await _retryPolicy.ExecuteAsync(async () =>
        {
            ...
            _logger.LogInformation("Submitted {FileName} to Sandata", request.FileName);
    ...
    {
        return Task.FromResult(Result.Failure<EligibilityResponse>("Sandata does not support real-time eligibility"));
    }
...
public class SandataConfiguration
{
