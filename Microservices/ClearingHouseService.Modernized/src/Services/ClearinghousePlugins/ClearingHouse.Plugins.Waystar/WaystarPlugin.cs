...
namespace ClearingHouse.Plugins.Waystar;

public class WaystarPlugin : IClearinghousePlugin, IDisposable
{
    private readonly WaystarConfiguration _config;
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly ILogger<WaystarPlugin> _logger;
    private readonly AsyncRetryPolicy _retryPolicy;

    public string ClearinghouseId => "Waystar";
    public string DisplayName => "Waystar Revenue Cycle Technology";
    public bool IsEnabled => _config.IsEnabled;

    public WaystarPlugin(WaystarConfiguration config, IHttpClientFactory httpClientFactory, ILogger<WaystarPlugin> logger)
    {
        _config = config;
    ...
    public async Task<Result> ValidateConnectionAsync(CancellationToken cancellationToken = default)
    {
        return await _retryPolicy.ExecuteAsync(async () =>
        {
            ...
            var result = client.IsConnected ? Result.Success() : Result.Failure("Waystar connection failed");
            client.Disconnect();
            return result;
        });
    }
    ...
    public async Task<Result<SubmissionResult>> SubmitClaimAsync(ClaimSubmissionRequest request, CancellationToken cancellationToken = default)
    {
        return await _retryPolicy.ExecuteAsync(async () =>
        {
            ...
            _logger.LogInformation("Submitted {FileName} to Waystar", request.FileName);
    ...
    {
        _logger.LogInformation("Waystar eligibility requires REST API - not yet integrated");
        return Task.FromResult(Result.Failure<EligibilityResponse>("Waystar REST API eligibility integration pending"));
    }
...
public class WaystarConfiguration
{
